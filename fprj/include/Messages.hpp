...
template <typename T> T receiveMessage(std::string messageQueueName)
{
  size_t       recvd_size;
  ...
  do
  {
    ...
    catch (const interprocess_exception &ex)
    {
      ...
      std::cout << ex.what() << " receiveMessage: \"" << messageQueueName
                << "\"" << std::endl;
      exit(EXIT_FAILURE);
    }
    ReceiveMtx.unlock();
...
template <typename T> void SendMessage(std::string messageQueueName, T msg)
{
  SendMtx.lock();
  unsigned int priority = 0;
  try
  ...
  {
    message_queue::remove(messageQueueName.c_str());
    std::cout << ex.what() << " SendMessage: \"" << messageQueueName << "\""
              << std::endl;
    exit(EXIT_FAILURE);
  }
  SendMtx.unlock();
