...
public:
	...
	void ts_handleAirplane()
	{
		...
		try
		{
			message_queue planeToTower_(open_or_create, "tower_msgs", 100, sizeof(Messages::PlaneToTower));
			message_queue::size_type received_size;
			unsigned int priority = 1;
			std::cout << "Waiting for package" << std::endl;
			planeToTower_.receive(&plane2towerPackage_, sizeof(plane2towerPackage_), received_size, priority);
			std::cout << "Recieved package from plane " << plane2towerPackage_.TailNr << std::endl;
			...
			default:
				std::cout << "Tower: Received packet from plane: "
...
private:
	void searchForFreeRunway(std::vector<runway*>& RW, bool& permission, runway** currentRunwayPtr)
	{
