...
public:
	...
	void emptyAirplane(airplane<CargoCapacity, PassengerCapacity, Size, Reach>& plane, std::vector<passenger>& pass)
	{
		std::vector<passenger> tempVec;
		tempVec.swap(plane.getPassengers());		
		pass.insert(pass.end(), std::make_move_iterator(tempVec.begin()), std::make_move_iterator(tempVec.end()));
	};

	template<int CargoCapacity, int PassengerCapacity, int Size, int Reach>
	void fillAirplane(airplane<CargoCapacity, PassengerCapacity, Size, Reach>& plane, std::vector<passenger>& pass)
	{
