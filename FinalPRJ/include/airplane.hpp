...
public:
	airplane(std::string Position, std::string TailNumber, bool leaving = true)
		: position_(Position), tailNumber_(TailNumber), currentRunway_(nullptr), leaving_(leaving)
	{
	...
	void ts_takeOff()
	{
		...
		try
		{
			...
			message_queue fromTowerToPlane_(open_or_create, tailNumber_.c_str(), 100, sizeof(Messages::TowerToPlane));
	...
	void ts_touchDown()
	{
		...
		try
		{
			...
			do
			{
				...
				{
					boost::unique_lock<boost::mutex> scoped_lock(mutex_);
					std::cout << tailNumber_ << " sent a message at touchdown" << std::endl;
				}
				fromTowerToPlane_.receive(&receiveMsg, sizeof(fromTowerToPlane_), received_size, priority);
				{
					boost::unique_lock<boost::mutex> scoped_lock(mutex_);
			...
			} while (!receiveMsg.Permission); // Continue to ask for permission while permission is no granted!
			currentRunway_ = &receiveMsg.RW;
			{
				boost::unique_lock<boost::mutex> scoped_lock(mutex_);
				std::cout << tailNumber_ << " has landed on runway" << currentRunway_->getRunwayNumber() << std::endl;
			}
		}
		catch (const interprocess_exception& ex)
		{
	...
	void ts_taxi()
	{
		...
		try
		{
			...
			{
				mutex_.lock();
				std::cout << tailNumber_ << " sending request to taxi to empty runway" << std::endl;
				mutex_.unlock();
				fromPlaneToTower_.send(&sendMsg, sizeof(sendMsg), 0);
				fromTowerToPlane_.receive(&receiveMsg, sizeof(fromTowerToPlane_), recieved_size, priority);
				mutex_.lock();
				std::cout << tailNumber_ << " received a message from tower" << std::endl;
				mutex_.unlock();
				boost::this_thread::sleep_for(boost::chrono::microseconds(std::rand() % 20));
			...
			if (leaving_)
			{
				boost::unique_lock<boost::mutex> scoped_lock(mutex_);
				currentRunway_ = &receiveMsg.RW;
				std::cout << tailNumber_ << " is taxiing to runway" << currentRunway_->getRunwayNumber() << std::endl;
			}
			else
			{
				boost::unique_lock<boost::mutex> scoped_lock(mutex_);
				std::cout << tailNumber_ << " is taxiing from runway" << currentRunway_->getRunwayNumber() << std::endl;
			}
		}
		catch (const interprocess_exception& ex)
		{
