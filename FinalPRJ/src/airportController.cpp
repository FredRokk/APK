...
void airplaneThread(airplane<CargoCapacity, PassengerCapacity, Size, Reach> *plane, std::vector<gate> *gates_, std::vector<passenger> *passengers)
{
	again:
	boost::mutex mtx;
	if(plane->isLeaving())
...
int main()
{
	...
	boost::thread testThread1(&airplaneThread<100, 200, 1500, 13000>, &test, &gates, &passVec);
	boost::thread testThread2(&airplaneThread<100, 200, 1500, 13000>, &test2, &gates, &passVec);
